...
public class BM25Indexer {
	...
	private static void indexer(LinkedHashMap<String, ArrayList<DocInfo>> ind, LinkedHashMap<Integer, Integer> dl, String ip_file, String out_file, Integer documentId) {
		...
		try 
		{
			...
			while ((sCurrentLine = input.readLine()) != null) 
			{
				...
				for(int i = 0 ; i < str.length ; i++)
				{
					...
					dl.put(documentId,documentId+1);
				}//end of for
			}// end of while				
		} 
		catch (IOException e) 
			...
			String query_file, String result_file, int numOfDocs) 
	{
		...
		try {
			...
			while ((txtLine = input.readLine()) != null) {
				 ...
				 * and its corresponding occurrence in the query text
				 */
	...
	{		
		...
		final File folder = new File("RawDocs");
		...
		{	        
        	...
        	indexer(ind,dl,"RawDocs/" + fileEntry.getName(),"index.out", documentId);
	    }
